...
( function( $ ) {
    ...
    $.hestiaCustomizeLive = {
        ...
        'liveTextReplace': function () {
            var textToReplace = [
                ...
                { controlName:'hestia_contact_area_title', selector:'.hestia-contact .card-contact .card-title' },
                { controlName:'hestia_blog_subscribe_title', selector:'#subscribe-on-blog h3.hestia-title' },
                { controlName:'hestia_blog_subscribe_subtitle', selector:'#subscribe-on-blog p.description' }
            ];
    ...
    wp.customize(
        'accent_color', function( value ) {
            value.bind(
                function( newval ) {
                    $( '.main section:not(.hestia-blogs) a:not(.btn):not(.blog-item-title-link):not(.shop-item-title-link):not(.moretag):not(.button), .card-product .category a, .navbar.navbar-color-on-scroll:not(.navbar-transparent) li.active a' ).css( 'color', newval );
