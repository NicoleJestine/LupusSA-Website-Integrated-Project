...
class Hestia_About_Page {
	...
	/**
	 * The single instance of Hestia_About_Page
	 *
	 * @var Hestia_About_Page $instance The Hestia_About_Page instance.
	 */
	...
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Hestia_About_Page ) ) {
			self::$instance = new Hestia_About_Page();
	...
	/**
	 * Check if recommended actions are completed
	 *
	 * @return bool
	 */
	...
	public function display_button( $data ) {
		...
		if ( $data['title'] !== 'Recommended actions' || ! $this->recommended_actions_completed() ) {
			echo '<a target="' . $button_new_tab . '" href="' . $data['button_link'] . '"class="' . esc_attr( $button_class ) . '">' . $data['button_label'] . '</a>';
		} else {
			echo '<span>' . $data['button_label'] . '</span>';
		}
	}
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'ti-about-page-css', get_template_directory_uri() . '/inc/admin/about-page/css/ti_about_page_css.css', array(), HESTIA_VERSION );
			...
			wp_enqueue_script(
				'ti-about-page-js',
				get_template_directory_uri() . '/inc/admin/about-page/js/ti_about_page_scripts.js',
				array(
					'jquery',
					'jquery-ui-tabs',
				),
	...
	public function recommended_actions_render() {
		...
		foreach ( $actions as $action_key => $action_value ) {
			...
			echo $this->get_feature_title_and_description( $action_value );

			if ( ! empty( $action_value['plugin_slug'] ) ) {
				$slug = $this->check_plugin_slug( $action_value['plugin_slug'] );
				?>
				<?php echo Hestia_Plugin_Install_Helper::instance()->get_button_html( $slug ); ?>
